...
understood programming best practices.

\begin{comment}
a desire to make available to composers the considerable amount
of work already accomplished by software engineers. There are surely more
programmers than composers in the world, and likely always will be: in deciding
to architect Abjad as an extension to one of the most widely known programming
languages in the world we are implicitly making the case that necessarily
relatively smaller global community of composers will be well-served in
adopting the tools and best practices of the much larger global community of
software developers and engineers.

Python is an excellent general-purpose programming
language. But our decision to implement all recent versions of Abjad in Python
does not arise from any of the specifics of the language itself but rather from
the fact that the language is supported by a large and active community of
users and that all features of the language are already well-documented and
explained by people other than ourselves.

currently in use we hope to ease the composers'
transition to programming in making available the global community of Python
language users. High-quality resources for learning Python are available in
all major languages and written according to a variety of different
perspectives and understandings. This

with Abjad is really a type of specialized programming working with Abjad while
keeping available to composers the entirety of the langauge's
features.\footnote{The core features of Python include fundamental data
structures like lists and dictionaries; the basics of flow control, looping and
iteration; the way code in one file refers to code in another file; and the
extensive functionality included as part of the language's standard library.}
Our insistence on this principle of extension --- of adding functionality to an
existing system rather than constructing from the ground up --- is fundamental
to way we have architected Abjad and the work underlying language constructs.

All versions of Abjad released since 2008 have
all been implemented in Python and architected in such a way as to make
available to composers all features of the Python programming language (rather
than only a subset of language features).\footnote{Early versions of
Abjad were implemented in \texttt{C} and \texttt{Mathematica}.}

music people always think they need a dsl.
everybody always makes that mistake. extend existing language. [TREVOR]
\end{comment}
