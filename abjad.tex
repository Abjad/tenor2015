...
By architecting Abjad as an extension to one of the most widely-used programming languages in the world, we hope to make the considerable power of programming available to composers in a straightforward a way. Abjad is implemented as a Python package;\footnote{See the Python Package Index for extensions to Python for purposes as diverse as creative writing and aeronautical engineering. The Python Package Index contains 54,306 packages at the time or writing and is available at \textit{https://pypi.python.org}.} composers work with Abjad exactly the same way developers work with all the other packages available for the language. This means that Abjad turns all or part of the work of composition into programming and that composers who work with Abjad structure parts of their musical thought as computer code. In the most common case this means opening a file, writing code and saving the file:
...
interactive console because the console helps keep input and output clear in example code.\footnote{Lines preceded by the \texttt{>>>} prompt are passed to Python for interpretation; any output generated by the line of code appears immediately after. The example above creates a tuplet with the tuplet-making function defined earlier and calls Abjad's top-level \texttt{show()} function to generate a PDF of the result.} But note that the primary way composers work with the system is by typing notationally-enable Python code into a collection of files and managing the result as a project grows to encompass the composition of an entire score.
...
%This means that composers who decide to model working with Abjad have access to all the features of one of the most widely-used and fully-developed programming languages ever invented.
