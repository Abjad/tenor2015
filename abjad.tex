...
% , any of which can be customized or sub-classed,
Abjad implements many different factories and composers can extend the system as they work to implement their own models of composition. Abjad also provides a variety of factory functions and factory classes for
facilitating top-down construction. These
range from simple note-generating functions, like \texttt{make\_notes()}, which
...
\begin{comment}
Factories generalize and encapsulate
processes to enable the parametric exploration of material.
\end{comment}
...
>>> show(rhythm_maker, divisions=divisions)
\end{lstlisting}
\includegraphics{assets/lilypond-358b2f51787297a1dbc4423c1c37374e.pdf}
%%% ABJADBOOK END %%%
...
\section{Synthesis: bottom-up \& top-down together}

Another common factory pattern in Abjad, score templates encapsulate the
...
for index, voice in enumerate(iterator):
    seed = -index
    selections = rhythm_maker(divisions, seeds=seed)
...
\noindent Factories generalize and encapsulate
processes to enable the parametric exploration of material.
...
>>> show(score)
\end{lstlisting}
\includegraphics{assets/lilypond-0a01efa1748eaea2385c849e49797cc2.pdf}
%%% ABJADBOOK END %%%
