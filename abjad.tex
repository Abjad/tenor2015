...
notation, or focusing on the abstract modeling of composition without a considered
link to a model of notation. Generative task explains a given system's balance between
...
systems \cite{Good:2001if}.\footnote{An attempt to survey more comprehensively the history of object-oriented notation systems for
...
\textit{https://pypi.python.org}.}\footnote{Designing Abjad as an extension to Python makes hundreds of print and Web resources relevant to composers working in Abjad. The global
...
\textit{leaf} to refer generally to notes, rests, and chords. This term, borrowed from
...
\emph{forte} until another note is marked \emph{piano}. The
scoping behavior of indicators is generalized to allow composers to annotate score structures
...
measures or voices that may then be included in even higher-level containers,
such as staves and scores.\footnote{Unlike many notation packages, Abjad does
not require composers to structure music into measures. All of Abjad's containers can hold leaves notes, rests and chords directly.}\footnote{Notes, rests and chords may be initialized with pitches named according to either American or European conventions, with the pitch numbers of American pitch-class theory or from combinations of Abjad pitch and duration objects.} Abjad implements this style of component aggregation via a
...
The literature investigated in preparing this report
...
Python's standard library. Parameterized tests ensure that
different classes implement similar behaviors in a consistent way.
Developers run the entire battery of tests at the start
...
Testing benefits project continuance when the original developers of a
...
modeling the musical ideas that composers use notation to
...
conversion, audio synthesis, realtime applications or graphic user interface
...
encourage composers working with Abjad to transition from working with
...
comments and contributions to the code. We would also like to thank
everyone behind the LilyPond and Python projects, as well as the wider
open-source community, for fostering the tools that make Abjad possible.
