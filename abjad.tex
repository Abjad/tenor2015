...
system that affords a detailed model of composition without linking to a
a detailed model of musical notation suffices for most tasks --- sufficiency, however, depends heavily on generative task. For example, if a composer requires an automated notation system to render complex rhythmic ideas that depend typographically on nested tuplets, a system that
...
Finale/Sibelius -- access to a model of notation with no access to model of composition.

lossy downsampling of information representation due to MIDI somewhere in pipeline -- potentially detailed models of either composition or notation

insufficient notation model - too many typographical details aren't modeled, working in Illustrator

composition, not notation
notation, not composition
losing information because of representation
...
composition \cite{Smith:1972mw}, \cite{Nienhuys:2003ve},
...
relatively more genre-agnostic software tools. These tools provide detailed models of notation without allowing equivalently detailed models of composition, although such systems might go so far as
to enable a text-based object-oriented model of notation to automate some
...
The music software systems academic literature we have investigated is overwhelmingly silent on questions of software testing. None of the sources cited in this article reference software test methodologies. The same appears to be true for the much larger list of sources included in \cite{Trevino:2013aa}\footnote{Chris Ariza's work developing AthenaCL [SOURCE] and the development of Music21 [SOURCE] by Michael Cuthbert are important exceptions. Both projects are implemented in Python and both are aware of the the collection of best practices outlined in this section.} Why should this be the case? One possibility is that authors of music software systems have, in fact, availed themselves of the important improvements in software test methods developed over the previous decades but have, for whatever reasons, remained quiet on the matter in the publication record. We think a more likely explanation is that the culture of software best practices now widely followed in the open-source community simply has not yet arrived in the field of music software systems development (especially in the development of music software systems designed for noncommercial applications). This is a situation we work strongly to avoid in the development of Abjad.
Our experience developing Abjad leads us to believe that the focus on testing absent the majority of music software systems is a liability to the field in two ways.\footnote{Abjad comprises an automated battery of 9,119 unit tests and 8,528 documentation tests. Unit tests are executed by \texttt{pytest}. Documentation tests are executed by the \texttt{doctest} module included in Python's standard library. Parametrized tests are provided to ensure that behaviors implemented the same way against different classes produce corresponding output. Developers run the entire battery of tests at the start of every development session. No new features are accepted as part of the Abjad codebase without tests authored to document changes to the system. Continuous integration testing is handled by Travis CI to ensure that nightly builds of Abjad installed under different operating systems continue to pass all tests. For \texttt{pytest} see \textit{http://pytest.org}. For Travis CI see \textit{https://travis-ci.org}.}
The first such liability concerns systems development. The absence of automated regression tests acts as a barrier to new contributions to the system (who effectively have no contract to document expected behavior of the system) and severely attenuates the rate at which experienced developers can refactor the system during new feature development. Consider that Abjad currently comprises about 178,000 lines of code but that the Abjad repository hosted on GitHub\footnote{https://github.com/Abjad/abjad} lists more than 8.7 million lines of code committed since the start of the project. This refactor
ratio of about 50:1 means that each line of code in the Abjad codebase has been rewritten an average of nearly 50 times. This rate of refactoring is possible only because of the approach to automated regression testing we have borrowed from the larger open-source community; without such an approach we think the development of other complex music software systems will be strongly limited.
The second liability we diagnose in the relative absence of automated regression in music software systems concerns project continuance. What happens when the original developers of a music software system are no longer available to develop the system? The questions becomes increasingly acute when the system in question incorporates proprietary components, isn't documented, has had a small number of contributors and especially when the system contains no well-structured battery of tests. Automated tests serve as a type of behavioral contract for the system that serve to make possible a changing of the guard from one set of developers to another over what may potentially be the many decades of a software system's lifetime. The absence of such a test battery makes this sort of hand-off much more difficult and, in the worst case, nearly impossible. We hope that when the time comes for us to hand over primary responsibilities for the development of Abjad that the extensive collection of tests we have developed for the system will help ensure the continuance of the project as a whole.
