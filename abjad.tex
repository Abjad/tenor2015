...
NoteAbility \cite{hamel1noteability}, and Nightingale --- exist to help people engrave and format music documents; because these systems provide functions that operate on
...
enable users to engrave and format music without exposing any particular underlying model of composition, and without requiring, or even inviting the user to computationally model composition. Such systems might go so far as to enable scripting, as in the case of Sibelius's ManuScript \cite{Technology:qc} scripting language or Lilypond's embedded Scheme code; although these systems enable the automation of notational elements, it remains difficult to model compositional
...
\section{Motivations}
Provide a sufficiently detailed model of model of common practice notation that simplifies the modeling of high-order processes and relationships to enable the user to model composition.
