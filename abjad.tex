...
ways: by name, numeric indices or iteration. A single operation can be mapped onto the entirety of a selection. Ad hoc intermediate structures can be created as they become
...
explicit names, the lower staff can be selected by name:
...
\noindent Using numeric indices, the upper staff can be selected by indexing the first child of the
first child of the score:
...
\noindent The top-level \texttt{iterate()} function exposes Abjad's score iteration interface. This interface
provides a collection of methods for iterating over the components in a score
according to various criteria. Such criteria include component
class, groups of tied notes and attack point order:\footnote{Abjad uses the term \emph{logical tie}
to refer to a selection of one or more leaf components which represent all of the
