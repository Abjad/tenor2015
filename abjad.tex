...
By architecting Abjad as an extension to one of the most widely-used programming languages currently in use, we hope to make the considerable power of programming available to composers in as straightforward a way as possible. Abjad is implemented as a Python package; composers work with Abjad exactly the same way Python developers work with all the other packages available for the language.\footnote{See the Python Package Index for packages to extend Python for purposes as diverse as creative writing and aeronautical engineering. The Python Package Index contains 54,306 packages at the time or writing and is available at \textit{pypi.python.org}.}
